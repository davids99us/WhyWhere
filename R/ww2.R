...
Bradypus_files <- list.files(path=paste(system.file(package="dismo"),'/ex',sep=''), 
  ...
  for (i in files) {
    ...
    if (is.null(a)) { print(paste("Skipping: membership failed")) }
      ...
      result=rbindlist(list(result,
                            data.table(name=a$name,AUC=a$AUC,file=i)))
      setorder(result,-AUC)
      ...
      if (a$name==result[1]$name) {
        ...
        if (plot) {
          browser()
          plot(predict.membership(a),main=paste(a$name," AUC=",a$AUC,sep=""))
      ...
      if (multi && auc>result[1]$AUC) {
        ...
        result=rbindlist(
          list(result,data.table(
            name=paste(result[1]$name,".",a$name,sep=""),AUC=auc,file=i)))
      }
      setorder(result,-AUC)
